...
#         return JSONResponse(content={"message": "Files uploaded successfully and processing started."})
#     except Exception as e:
#         raise HTTPException(status_code=500, detail=f"Upload failed: {str(e)}")

# # # Query endpoint
# # @router.post("/query/")
# # async def query_answer(query: str = Body(..., embed=True)):
# #     try:
...
#     query: str
#     doc_id: str = None  # Optional, but frontend sends this always

# @router.post("/query/")
# async def query_answer(request: QueryRequest):
#     try:
#         answer = handle_query(request.query, request.doc_id)
#         return JSONResponse(content={"answer": answer})
#     except Exception as e:
#         raise HTTPException(status_code=500, detail=f"Query failed: {str(e)}")
...
#             raise HTTPException(status_code=404, detail="No extractable text found in selected PDFs.")

#         theme_result = identify_themes(combined_text)

#         themes = theme_result.get("themes", [])
#         synthesized_answer = ". ".join(themes) if themes else "No themes found."
...
#         file_list = [{"filename": f} for f in files if f.lower().endswith(".pdf")]
...
#         if file_path.exists():
#             os.remove(file_path)
#             return JSONResponse(content={"message": f"{filename} deleted successfully."})
#         else:
...
#   if (!query || !doc_id) {
#     return res.status(400).json({ error: 'Missing query or doc_id' });
#   }

#   try {
#     // Fetch the document by doc_id
#     const document = await Document.findById(doc_id);
#     if (!document) {
#       return res.status(404).json({ error: 'Document not found' });
#     }

#     // Run your query logic on this document
#     // e.g., query the vector store or database filtered by this doc_id
...
# // Assume you have a Document model representing your uploaded documents

# app.get('/api/documents/list', async (req, res) => {
#   try {
#     const documents = await Document.find({}, '_id filename'); // return only _id and filename
#     res.json(documents);
#   } catch (err) {
